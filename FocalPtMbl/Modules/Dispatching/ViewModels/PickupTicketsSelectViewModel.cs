...
namespace FocalPoint.Modules.Dispatching.ViewModels
{
    public class PickupTicketsSelectViewModel : ThemeBaseViewModel
    {
        ...
        internal PickupTicket GetDetailedTicketInfo(PickupTicket SelectedTicket)
        {
            ...
            try
            {
                /*var response = PickupTicketEntityComponent.GetPickupTicket(SelectedTicket.PuTNo.ToString());
                var detailedTicket = response.Result;
                if (detailedTicket != null)
                {
                    bool isLocked = PickupTicketEntityComponent.LockPickupTicket(SelectedTicket.PuTNo.ToString(), isTrue.ToString()).GetAwaiter().GetResult();
                    if (isLocked)
                    {
